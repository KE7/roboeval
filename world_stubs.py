...
class BaseWorldStub:
    ...
    def refresh_objects(self, image: Optional[Image.Image]):
        ...
        if image is not None:
            ...
            self.manipulable_object_uids = vlmi.get_object_uids_from_scene(
                image, instruction
            )
    ...
    def ask_tf(self, question: str) -> bool:
        ...
        """
        answer = vlmi.evaluate_tf_question(question, self.current_image)
        if self.execution_trace is not None:
            self.execution_trace.record_reasoning(
                self.current_image, "ask_tf", question, answer
            )
        return answer
    ...
    def ask_mc(self, question: str, options: List[str]) -> str:
        ...
        """
        answer = vlmi.evaluate_mc_question(
            question, self.current_image, options_list=options
        )
        if self.execution_trace is not None:
            self.execution_trace.record_reasoning(
    ...
    def ask_question(self, question: str, options: Optional[List[str]] = None):
        ...
        """
        if options is None:
            return vlmi.evaluate_open_question(question, self.current_image)
    ...
    def act(self, command: str):
        ...
        raise NotImplementedError(
            "act() must be implemented by subclasses."
        )

    def reset(self, new_task: Optional[str] = None, keep_frames: bool = False):
    ...
    def physical_reset(self):
        ...
        """
        raise NotImplementedError(
            "physical_reset() must be implemented by subclasses."
        )
