"""Unit tests for roboeval.results (collector and merge).
...
from roboeval.results.collector import (  # tests adjust sys.path before importing local package.
    EpisodeResult,
    ResultCollector,
)
from roboeval.results.merge import (  # tests adjust sys.path before importing local package.
    find_shard_files,
    load_shard_files,
    merge_shards,
)
...
def _make_shard(
    ...
    if episodes is None:
        ...
        episodes = [{"episode_id": ep_id, "metrics": {"success": True}, "steps": 50}]
    result = {
        "benchmark": benchmark,
...
class TestMergeShards:
    ...
    def test_all_shards_present(self):
        ...
        for i in range(4):
            shards.append(
                _make_shard(
                    i, 4, episodes=[{"episode_id": i, "metrics": {"success": True}, "steps": 50}]
                )
            )
    ...
    def test_missing_shard_marks_partial(self):
        ...
        shards = [
            _make_shard(
                0, 3, episodes=[{"episode_id": 0, "metrics": {"success": True}, "steps": 50}]
            ),
            _make_shard(
                2, 3, episodes=[{"episode_id": 2, "metrics": {"success": True}, "steps": 50}]
            ),
            # shard 1 is missing
        ]
        merged = merge_shards(shards)
    ...
    def test_duplicate_episode_id_last_write_wins(self):
        ...
        shard_a = _make_shard(
            0, 2, episodes=[{"episode_id": 0, "metrics": {"success": False}, "steps": 10}]
        )
        shard_b = _make_shard(
            1,
            2,
    ...
    def test_metric_aggregates_recomputed(self):
        ...
        for i in range(4):
            shards.append(
                _make_shard(
                    i, 4, episodes=[{"episode_id": i, "metrics": {"success": i < 2}, "steps": 50}]
                )
            )
        merged = merge_shards(shards)
    ...
    def test_merge_info_fields(self):
        shards = [
            _make_shard(
                i, 3, episodes=[{"episode_id": i, "metrics": {"success": True}, "steps": 10}]
            )
            for i in range(3)
        ]
        merged = merge_shards(shards)
...
class TestLoadShardFiles:
    ...
    def test_find_shard_files_glob(self, tmp_path):
        for i in range(3):
            (tmp_path / f"result_shard{i}of3.json").write_text(json.dumps(_make_shard(i, 3)))
...
class TestRoundTrip:
    def test_collector_to_json_and_back(self):
        ...
        for ep in range(3):
            col.record(
                "task_0", EpisodeResult(episode_id=ep, metrics={"success": ep > 0}, steps=100)
            )
        js = col.to_json()
        data = json.loads(js)
    ...
    def test_merge_and_json_serializable(self):
        shards = [
            _make_shard(
                i, 2, episodes=[{"episode_id": i, "metrics": {"success": True}, "steps": 50}]
            )
            for i in range(2)
        ]
        merged = merge_shards(shards)
