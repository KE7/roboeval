...
def load_shard_files(paths: list[Path]) -> list[dict[str, Any]]:
    ...
    for p in paths:
        ...
        if "shard" not in data:
            raise ValueError(
                f"{p}: not a shard result file (missing 'shard' field)"
            )
        shards.append(data)
    return shards
...
def merge_shards(shards: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for s in shards:
        if s["benchmark"] != benchmark_name:
            raise ValueError(
                f"Benchmark mismatch: {s['benchmark']!r} vs {benchmark_name!r}"
            )
        if s["shard"]["total"] != expected_total:
            raise ValueError(
                f"Shard total mismatch: {s['shard']['total']} vs {expected_total}"
            )
        ...
        for task_result in shard.get("tasks", []):
            ...
            for ep in task_result.get("episodes", []):
                ...
                if ep_id in all_episodes[task_name]:
                    logger.warning(
                        "Duplicate episode_id %r in task %r "
                        "(shard %s overwrites previous)",
                        ep_id,
                        task_name,
                        shard_id,
                    )
    ...
    shard_dates = sorted(
        s.get("created_at", "") for s in shards if s.get("created_at")
    )
    if shard_dates:
...
def _print_merge_report_rich(con: Any, merged: dict[str, Any]) -> None:
    ...
    from robo_eval.results.collector import print_task_table
    ...
    if missing:
        con.print(
            f"\n[yellow]Missing shards: {missing} "
            f"(expected 0..{total_shards - 1})[/yellow]"
        )
        con.print(
            f"Coverage: {total_eps} episodes "
            f"(shards {len(found)}/{total_shards})"
        )
        con.print(
            f"Merged result ([yellow]PARTIAL[/yellow]): "
            f"[{rate_color}]{rate:.1%}[/{rate_color}]"
        )
    else:
        con.print(
            f"\n[green]All {total_shards} shards complete.[/green] "
