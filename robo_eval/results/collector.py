...
class EpisodeResult(TypedDict):
    ...
    ``metrics`` holds benchmark-defined values (e.g. ``success``, ``steps``).
    """
...
def _build_task_result(task_name: str, episodes: list, metric_keys: dict[str, str]) -> TaskResult:
    ...
    All episodes count toward metrics equally.
...
def print_task_table(console: Any, tasks: list, rate: float, rate_color: str) -> None:
    ...
    if total_errors:
        console.print(
            f"  [yellow]{total_errors} episodes had errors — "
            f"success rate may be understated[/yellow]"
        )
...
class ResultCollector:
    ...
    Records are organized hierarchically by episode, task, and benchmark.
    ...
    def __init__(
        ...
        self.mode = mode
        self.metric_keys = metric_keys or {"success": "mean"}
    ...
    def get_task_result(self, task_name: str) -> TaskResult:
        ...
        return _build_task_result(
            task_name, self._episodes.get(task_name, []), self.metric_keys
        )
    ...
    def get_benchmark_result(self, config: dict[str, Any] | None = None) -> BenchmarkResult:
        ...
        # Add benchmark-level aggregates using the configured metric keys.
        if self.metric_keys:
    ...
    def print_summary(self) -> None:
        ...
        try:
            ...
            print(f"\nBenchmark: {result['benchmark']} (mode: {result['mode']})")
