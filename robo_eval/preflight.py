"""Preflight smoke tests for robo-eval.

Implements ``robo-eval test``:
...
def check_server(server: dict[str, Any]) -> list[CheckResult]:
    ...
    try:
        ...
        for key, raw in action_spec_raw.items():
            ActionObsSpec.from_dict(raw)
        for key, raw in obs_spec_raw.items():
            ActionObsSpec.from_dict(raw)
...
def run_preflight(
    ...
    if all_checks:
        ...
        try:
            ...
            cfg.max_tasks = 1
            dry_dir = Path(results_dir or "/tmp/robo_eval_preflight_dry")
    ...
    if failed_count == 0:
        ...
        print(
            f"{_RED}{failed_count}/{total_count} checks failed.{_RESET}"
        )
