...
Usage::
    ...
    from roboeval.specs import ActionObsSpec, POSITION_DELTA, ROTATION_AA, GRIPPER_CLOSE_POS
...
def check_specs(
    ...
    if server_action and bench_action and not (server_action.keys() & bench_action.keys()):
        results.append((HARD, "action: no overlapping keys between server and benchmark specs"))
    ...
    for key in server_action.keys() & bench_action.keys():
        ...
        if prod.dims > 0 and cons.dims > 0 and prod.dims != cons.dims:
            results.append((HARD, f"action [{key}]: dims mismatch {prod.dims}D vs {cons.dims}D"))
            continue
        ...
        if prod.range and cons.range and prod.range != cons.range:
            results.append((WARN, f"action [{key}]: range mismatch {prod.range} vs {cons.range}"))
