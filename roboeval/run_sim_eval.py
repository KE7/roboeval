...
Prerequisites:
    1. Start the simulator server: ``roboeval serve --vla pi05 --sim libero --sim-port 5001``
    2. Start the litellm proxy:    ``bash scripts/start_vlm.sh``

Usage:
    python -m roboeval.run_sim_eval eval --sim libero --task 0 --suite libero_spatial \\
...
from roboeval.run import (
    get_reasoning_steps,
    get_top_level_task_assessment,
    save_video,
)
from roboeval.run_utils import save_reasoning_ica_dir, save_top_level_ica_dir
from sims.env_wrapper import VALID_SIMS, SimWrapper
...
app = typer.Typer(help="LITEN Simulator Evaluation: Run LITEN against robotic simulators.")
    ...
    for episode in range(start_episode, start_episode + max_episodes):
        ...
        try:
            ...
            if not ep_results_dir:
                ep_results_dir = str(Path(experience_dir).parent.parent)
            ...
            assessment = None
            if not no_vlm:
                typer.echo(f"Processing {len(wrapper.subtask_frame_tuples)} subtasks...")
                subtask_reasoning_tuples = []
                for i, (subtask_cmd, frames) in enumerate(wrapper.subtask_frame_tuples):
                    ...
                    subtask_reasoning_tuples.append((subtask_cmd, success, whathappened, reasoning))
