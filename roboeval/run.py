...
app = typer.Typer(help="Physical Robot CLI: Typer-based interface for running planner methods.")
...
def _assess_success_and_critique(
    ...
    if not success:
        if video_frames is None:
            whathappened = vlmi.critique_vla_failure(prior_image, final_image, instruction)
        else:
            whathappened = vlmi.critique_vla_video_failure(video_frames, instruction)
    return success, whathappened


def get_reasoning_steps(prior_image, final_image, instruction, video_frames=None, use_human=False):
    ...
    if success:
        ...
        reasoning = vlmi.reason_about_vla_failure(prior_image, instruction, whathappened)
    return success, whathappened, reasoning


def get_who_steps(prior_image, final_image, instruction, video_frames=None, use_human=False):
...
def get_top_level_task_assessment(
    ...
    """
    if success:
        return vlmi.assess_hl_success(prior_image, final_image, hl_instruction, reasoning_tuples)
    return vlmi.assess_hl_failure(prior_image, final_image, hl_instruction, reasoning_tuples)
...
@app.command("planner")
def planner(
    experiment_name: str = typer.Argument(help="Name of experiment (will be timestamped also)."),
    experience_dir_path: str = typer.Option(
        "planner_context",
        help="Path to past experience storage for in-context inclusion.",
    ),
    ask_save_video: bool = typer.Option(False, help="Offer to save higher-quality videos."),
):
    """
    ...
    while True:
        typer.echo("Input: A task instruction to execute by the VLA. Input 'exit' to break:")
        ...
        print(justification)
        print(llm_stats)
        typer.echo("Press Enter to execute the generated program, or type 'exit' to break.")
        is_ready = typer.prompt("Ready?", default="")
        if is_ready == "exit":
            break
        else:
...
def ablation_who(
    ...
    while True:
        ...
        for i, tupl in enumerate(world.subtask_frame_tuples):
            ...
            os.makedirs(unique_dir_pathname, exist_ok=True)
            save_who_ablation_dir(unique_dir_pathname, init_img, task, outcome, whathappened)
...
def reflexion_like(
    ...
    while True:
        ...
        all_frames = []
        all_tasks = []
        for _i, tupl in enumerate(world.subtask_frame_tuples):
            task, frames = tupl
            all_frames.extend(frames)
            all_tasks.append(task)
        print("Generating reflexion feedback...")
        reflection = vlmi.generate_reflexion_feedback(instruction, all_tasks, all_frames)
        save_reflexion_dir(
            overalltaskdir,
            initial_image,
            instruction,
...
def test_planner(
    ...
    task_instruction, example_image_path = get_task_info(instruction_input)
    initial_image = Image.open(example_image_path)
    manipulable_objects = vlmi.get_object_uids_from_scene(initial_image, task_instruction)
    typer.echo("Generating new plan program...")
    new_programplan, justification = vlmi.generate_planner_program(
        initial_image,
        task_instruction,
